...
[DXVK](https://github.com/doitsujin/dxvk) is a Vulkan-based translation layer for Direct3D 8/9/10/11 that translates such calls into Vulkan, which is an effecient cross platform graphics API that can reduce CPU bottlenecks. I suggest reading more about it in their own explanation [here](https://dxvk.org/can-dxvk-fix-stuttering-issues/). While this was initially developed for Linux Gaming, this can be used on Windows and may provide minimal improvements to the smoothness of the game. I recommend enabling it for any Linux users, as shown in the Linux Guide i wrote, which requires minimal effort to apply and use for all intended applications.
